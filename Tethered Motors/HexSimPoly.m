function [X,J,Z,S]=HexSimPoly(X,J,Z)
% This code is used to quickly record the 3 data
...
% analyzing. It also includes a noise concept of filament polymerization.
% 01/26/14
...
% Callie J Miller
% USES THE FOLLOWING FUNCTIONS: findfil2Dv2.m, newMotorPos.m
%
...
global Time M N L p0 p1 p2 h r v k k0 cote x0 y0 xhex yhex Zold t DeltaX dist
...
for t = 1:Time
%     % Motor Attaching/Detaching
%     for j = 1:M
%         if J(1,j) == 0 && J(2,j) ==0 %L and R leg of motor are unattached
%             for i=1:N
%                 count(i)=findfil2D(Z(1,i),Z(2,i),Z(4,i),Z(5,i),X(1,j),X(2,j),r); % see if there are filaments to attach to.
%             end
...
%             end
% 
%             clear count fil fil2 a indicies indicies2
%         elseif J(1,j) ==0 && J(2,j)~=0 %L leg motor is unattached but R leg is attached
%             for i=1:N % loop through all filaments to see if there are any close to motor j
%                 count(i)=findfil2D(Z(1,i),Z(2,i),Z(4,i),Z(5,i),X(1,j),X(2,j),r);
%             end
...
%                 X(4,j)=X(2,j);
%             end
%             clear count fil a indicies
%         elseif J(1,j)~=0 && J(2,j)==0 %L leg motor is attached but R leg is unattached
%             for i=1:N % loop through all filaments to see if there are any close to motor j
%                 count(i)=findfil2D(Z(1,i),Z(2,i),Z(4,i),Z(5,i),X(3,j),X(4,j),r);
%             end
...
%             end
%             clear count fil a indicies
%         elseif J(1,j)==J(2,j) && J(1,j)~=0 % Both R and L legs attached to the same filament
...
%             end
%             clear count fil a indicies
%         elseif J(1,j)~=0 && J(2,j)~=0 && J(1,j)~=J(2,j) % both R and L leg are attached to different filaments
%             if rand()>0.5
...
%     end
    ...
    for j=1:M
        ...
        elseif J(1,j)==0 && J(2,j) ~=0 && J(2,j)~=-1% L leg motor is unattached but R is attached
            [count]=findfil2Dv2(Z,X(1,j),X(2,j));
            if sum(count)==0 || p1*h<rand() % No filaments in the area or rate of attachment doesn't occur, then L leg of motor can't attach to any filament
                J(1,j)=0; % L motor stays unattached
                J(2,j)=J(2,j); % R motor stays attached to original filament
        ...
        elseif J(1,j)~=0 && J(2,j) ~=0 && J(1,j)~=J(2,j) && J(2,j)~=-1
            if rand()>0.5
                if p0*h>rand() % motor will detach
                    J(1,j)=0;
                    X(1,j)=X(3,j);
        ...
        elseif J(1,j)==0 && J(2,j)==-1
            [count]=findfil2Dv2(Z,X(1,j),X(2,j));
            if sum(count)==0 || rand()<Dif || p1*h<rand()%if these things are true, motor will stay unattached and X(1,i) and X(2,i) stays at the same place.
                X(1,j)=X(3,j);
                X(2,j)=X(4,j);
                J(1,j)=0;
                J(2,j)=-1;
            else % motor L leg can attach to a filament.
                a=find(count); %will find the index of the nonzero elements in count
                indicies=ceil(rand()*max(size(a))); % randomly pick one of these for L leg to bind
                fil=a(indicies);
                J(1,j)=fil;
    ...
    % MOTOR MOVEMENT
    for j=1:M
        if J(1,j)==0 && J(2,j)~=0 && J(2,j)~=-1% L motor is unattached R is attached
            for ii=1:N
                if J(2,j)==ii % if motor j is attached to filament i
                    S(ii,j)=sqrt((Z(1,ii)-X(3,j))^2+(Z(2,ii)-X(4,j))^2);
                    cb=sqrt((Z(4,ii)-X(3,j))^2+(Z(5,ii)-X(4,j))^2);
                    if cb>L % then motor has past the end of the filament and will fall off.
                        ...
                        S(ii,j)=S(ii,j)-v*h;
                        X(3,j)=Z(1,ii)-S(ii,j)*cos(Z(3,ii));
                        X(4,j)=Z(2,ii)-S(ii,j)*sin(Z(3,ii));
                        X(1,j)=X(3,j);
                        X(2,j)=X(4,j);
                    end
        ...
        elseif J(1,j)~=0 && J(2,j)==0 % R motor is unattached L is attached
            for ii=1:N
                if J(1,j)==ii % if motor j is attached to filament i
                    S(ii,j)=sqrt((Z(1,ii)-X(1,j))^2+(Z(2,ii)-X(2,j))^2);
                    cb=sqrt((Z(4,ii)-X(1,j))^2+(Z(5,ii)-X(2,j))^2);
                    if cb>L % then motor has past the end of the filament and will fall off.
                        J(1,j)=0;
                        % place motor at the plus end of the filament
                        X(1,j)=Z(1,ii);
                        ...
                        % motor continues to walk down the filament
                        S(ii,j)=S(ii,j)-v*h;
                        X(1,j)=Z(1,ii)-S(ii,j)*cos(Z(3,ii));
                        X(2,j)=Z(2,ii)-S(ii,j)*sin(Z(3,ii));
                        X(3,j)=X(1,j);
                        X(4,j)=X(2,j);
                    end             
                end
            ...
            cb=0;
        elseif J(1,j)~=0 && J(2,j)~=0 && J(1,j)~=J(2,j) && J(2,j)~=-1% R and L motor are attached to two different filaments
            for ii=1:N
                if J(1,j)==ii % motor j is attached to filament i
                    S(ii,j)=sqrt((Z(1,ii)-X(1,j))^2+(Z(2,ii)-X(2,j))^2);
                    cb=sqrt((Z(4,ii)-X(1,j))^2+(Z(5,ii)-X(2,j))^2);
                    if cb>L % then motor has past the end of the filament and will fall off.
                        J(1,j)=0;
                ...
                if J(2,j)==ii % motor j is attached to filament i
                    S(ii,j)=sqrt((Z(1,ii)-X(3,j))^2+(Z(2,ii)-X(4,j))^2);
                    cb=sqrt((Z(4,ii)-X(3,j))^2+(Z(5,ii)-X(4,j))^2);
                    if cb>L % then motor has past the end of the filament and will fall off.
                        J(2,j)=0;
        ...
        elseif J(1,j)==J(2,j) && J(1,j)~=0 %R and L motor are attached to the same filament
            for ii=1:N
                if J(1,j)==ii %found the filament both sides of motor are attached to
                    S(ii,j)=sqrt((Z(1,ii)-X(1,j))^2+(Z(2,ii)-X(2,j))^2);
                    cb=sqrt((Z(4,ii)-X(1,j))^2+(Z(5,ii)-X(2,j))^2);
            ...
            cb=0;
        elseif J(1,j)~=0 && J(2,j)==-1 % One of the tethered motors is attached to a filament so L motor will walk
            for ii=1:N
                if J(1,j)==ii % motor j is attached to filament i
                    S(ii,j)=sqrt((Z(1,ii)-X(1,j))^2+(Z(2,ii)-X(2,j))^2);
                    cb=sqrt((Z(4,ii)-X(1,j))^2+(Z(5,ii)-X(2,j))^2);
                    if cb>L % then motor has past the end of the filament and will fall off.
                        J(1,j)=0;
                        J(2,j)=-1;
                        % place motor at tethered position
                        X(1,j)=X(3,j);
                        X(2,j)=X(4,j);
                    else
                        % motor continues to walk down the filament
            ...
            motorStretch=sqrt((X(1,j)-X(3,j))^2+(X(2,j)-X(4,j))^2);
            if motorStretch>r % then motor has passed the threshold of stretch and falls off.
                J(1,j)=0;
                J(2,j)=-1;
                X(1,j)=X(3,j);
                X(2,j)=X(4,j);
    ...
    for ii=1:N %Only occurs for non-vertex filaments
        if p2*h > rand() % If this is true, then depolymerization event occurs causing attached motors to fall off, and filament to change plus end position and angle
            for j=1:M %Find any attached motors to the depolymerizing filament
                if J(1,j)==ii && J(2,j) ~=ii %if right leg of motor is attached to polymerizing filament i, but left leg is on a different filament
                    J(1,j)=0; % right leg unattaches
                    X(1,j)=X(3,j); %motor moves to the attached left leg position
                    X(2,j)=X(4,j);
                elseif J(2,j)==ii  && J(1,j)~=ii % if left leg of motor is attached to polymerizing filament i, and right leg is on a different filament
                    J(2,j)=0; % left leg unattaches
                    X(3,j)=X(1,j); % motor moves to the attached right leg position
                    X(4,j)=X(2,j);
                elseif J(1,j)==ii && J(2,j)==ii %if both legs are on the depolymerizing filament
            ...
            Z(1,ii)=(xhex(1)+(xhex(4)-xhex(1))*rand);
            Z(2,ii)=(yhex(2)+(yhex(5)-yhex(2))*rand);
            inPlus=inpolygon(Z(1,ii),Z(2,ii),xhex,yhex);
            while inPlus==0
                Z(1,ii)=(xhex(1)+(xhex(4)-xhex(1))*rand);
                Z(2,ii)=(yhex(2)+(yhex(5)-yhex(2))*rand);
                inPlus=inpolygon(Z(1,ii),Z(2,ii),xhex,yhex);
            ...
            Z(3,ii)=rand()*2*pi;
            Z(4,ii)=Z(1,ii)-L*cos(Z(3,ii));
            Z(5,ii)=Z(2,ii)-L*sin(Z(3,ii));
            inMinus=inpolygon(Z(4,ii),Z(5,ii),xhex,yhex);
    ...
    % FILAMENT MOVEMENT
    % movement for "Free" filaments
    for ii=1:N
        rot=[cos(Z(3,ii)),sin(Z(3,ii)); -sin(Z(3,ii)),cos(Z(3,ii))];
        rot_inv=[cos(Z(3,ii)),-sin(Z(3,ii));sin(Z(3,ii)),cos(Z(3,ii))];
        COMx=(Z(1,ii)+Z(4,ii))/2;
        COMy=(Z(2,ii)+Z(5,ii))/2;
        XR=rot*[COMx;COMy];
        len=zeros(1,M);
        fx=zeros(1,M);
        fy=zeros(1,M);
        for j=1:M
            if J(1,j)==ii && J(1,j)~=J(2,j) && J(2,j) ~=0 % both ends of motor are attached to 2 different filaments
                mxA(j)=X(1,j);
                myA(j)=X(2,j);
                mxB(j)=X(3,j);
                myB(j)=X(4,j);
                len(j)=sqrt((mxA(j)-COMx)^2+(myA(j)-COMy)^2);
                if S(ii,j)>L/2;
                    len(j)=-len(j);
                end
                x1(j)=COMx+len(j)*cos(Z(3,ii));
                y1(j)=COMy+len(j)*sin(Z(3,ii));
                fx(j)=k*(mxB(j)-x1(j));
                fy(j)=k*(myB(j)-y1(j));
            elseif J(2,j)==ii && J(1,j)~=J(2,j) && J(1,j)~=0
                mxA(j)=X(3,j);
                myA(j)=X(4,j);
                mxB(j)=X(1,j);
                myB(j)=X(2,j);
...
%             FR(:,j)=rot*F(:,j);
        ...
        %Update positions
        XR_n(1,1)=XR(1)+h/l_pr*sum(FR(1,:));
        XR_n(2,1)=XR(2)+h/l_pp*sum(FR(2,:));
        alpha_n=Z(3,ii)+h/l_r*sum(ang);

        %transform back
        COM_n=rot_inv*XR_n;
        Z(3,ii)=alpha_n;
        Z(1,ii)=COM_n(1,1)+(L/2)*cos(Z(3,ii));
        Z(2,ii)=COM_n(2,1)+(L/2)*sin(Z(3,ii));
        Z(4,ii)=Z(1,ii)-L*cos(Z(3,ii));
        Z(5,ii)=Z(2,ii)-L*sin(Z(3,ii));
        %update motors to stay on the newly relocated filament
        for j=1:M
            if J(1,j)~=J(2,j) && J(1,j)~=0 && J(2,j)~=0 && J(2,j)~=-1% both ends of motor are attached to different filaments
                if J(1,j)==ii 
                    X(1,j)=Z(1,ii)-S(ii,j)*cos(Z(3,ii));
                    X(2,j)=Z(2,ii)-S(ii,j)*sin(Z(3,ii));
                elseif J(2,j)==ii
            ...
            elseif J(1,j)~=J(2,j) && J(1,j)~=0 && J(2,j)==-1
                if J(1,j)==ii
                    X(1,j)=Z(1,ii)-S(ii,j)*cos(Z(3,ii));
                    X(2,j)=Z(2,ii)-S(ii,j)*sin(Z(3,ii));
                end
...
%     axis([-2 2 -2 2]);
%     for i=1:N
%         plot([Z(1,i),Z(4,i)],[Z(2,i),Z(5,i)],'k');
%         hold on
%         plot(Z(1,i),Z(2,i),'k.');
%     end
%     for j=1:M
%         plot([X(1,j),X(3,j)],[X(2,j),X(4,j)],'r');
%     end

...
global Link Time M N L p0 p1 p2 h r v k k0 cote x0 y0 xhex yhex Zold t DeltaX dist
...
for t = 1:Time
    ...
    % Crosslinker Attaching/Detaching
    for j=1:Link
        if J(1,j) == 0 && J(2,j)==0 && p1*h>rand() && rand()>Dif
            CL(1)=J(1,j);
            CL(2)=J(2,j);
            CL(3)=X(1,j);
            CL(4)=X(2,j);
    ...
    for j=Link+1:M
        if J(1,j)==0 && J(2,j)==0 % R and L legs of motor are unattached
            [count]=findfil2Dv2(Z,X(1,j),X(2,j),r(j));
        ...
        elseif J(1,j)~=0 && J(2,j)==0 % R leg motor is unattached but L is attached
            [count]=findfil2Dv2(Z,X(3,j),X(4,j),r(j));
    ...
    for j=1:M
        ...
        elseif J(1,j)~=0 && J(2,j)~=0 && J(1,j)~=J(2,j)% R and L motor are attached to two different filaments
            ...
            if motorStretch>r(j) % then motor as passed the threshold of stretch and falls off.
                J(1,j)=0;
                J(2,j)=0;
...
%         clear rot rot_inv COMx COMy XR mxA myA mxB myB F FR P_n COM_n
    ...
    fidCL=fopen(sprintf('CL%d.txt',t),'w');
    for j=1:Link
